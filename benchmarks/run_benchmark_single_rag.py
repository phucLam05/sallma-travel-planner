...
def single_agent_prompt(history: list[str]) -> list[Any]:
    system_prompt = (
        "Ban la mot travel planner single-agent co su dung RAG.\n"
        "Ban co quyen su dung cong cu retrieve_places de tim kiem thong tin khach san, nha hang, va diem tham quan tu database.\n"
        "Hay doc toan bo lich su yeu cau, goi cac cong cu (tool) can thiet de tim thong tin day du ve cac dia diem hop ly,\n"
        "sau do tong hop va tra ve phuong an cap nhat moi nhat duoi dang JSON hop le duy nhat.\n"
        "LUAT QUAN TRONG: CHỈ SỬ DỤNG DỮ LIỆU TỪ CONTEXT/TOOL. KHÔNG BỊA ĐỊA ĐIỂM. KHÔNG BỊA NHÀ HÀNG.\n"
        "Moi khi bat dau hoac refine, hay goi retrieve_places voi cac category khac nhau ('hotel', 'attraction', 'restaurant') de lay du lieu truoc khi tra ve JSON.\n"
        "JSON schema:\n"
        "{\n"
...
def run_single_agent(history: list[str]) -> dict[str, Any]:
    ...
    messages = single_agent_prompt(history)
    ...
    while response.tool_calls and iteration_count < MAX_ITERATIONS:
        ...
        if iteration_count >= MAX_ITERATIONS:
            messages.append(SystemMessage(content="Ban da dat gioi han so lan goi tool. Hay dung lai va tra ve JSON ket qua."))
...
def run_create_benchmark(
    ...
    print(f"[create] single-agent (RAG) on {len(prompts)} prompts", flush=True)
    for start in range(len(single_cases), len(prompts), batch_size):
        ...
        for offset, prompt in enumerate(batch, start=1):
            ...
            print(
                f"[create] single-agent (RAG) starting {prompt_index}/{len(prompts)}",
                flush=True,
            )
            case = run_single_create_case(prompt, places)
            single_cases.append(case)
            print(
                f"[create] single-agent (RAG) finished {prompt_index}/{len(prompts)} in {case['latency_seconds']:.3f}s",
                flush=True,
            )
        save_json(checkpoint_path, checkpoint_data)
        print(
            f"[create] single-agent (RAG) checkpoint {min(start + batch_size, len(prompts))}/{len(prompts)}",
            flush=True,
        )
...
def run_state_retention_benchmark(
    ...
    for session_index, session_turns in enumerate(
        ...
        save_json(checkpoint_path, checkpoint_data)
        print(f"[state] single-agent (RAG) session {session_index}/{len(MULTI_TURN_SESSIONS)} done", flush=True)
...
def write_markdown_report(path: Path, report: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "## 2. Consistency",
        "",
        f"- Single-agent (RAG) average consistency violations per create case: `{create_single['avg_consistency_violations']}`",
        f"- Multi-agent average consistency violations per create case: `{create_multi['avg_consistency_violations']}`",
        f"- Single-agent (RAG) average consistency violations in 10-turn sessions: `{state_single['avg_consistency_violations']}`",
        f"- Multi-agent average consistency violations in 10-turn sessions: `{state_multi['avg_consistency_violations']}`",
        "",
        "## 3. State Retention",
        "",
        f"- Single-agent (RAG) average state retention pass rate: `{state_single['avg_state_retention_pass_rate']*100:.2f}%`",
        ...
        "",
        f"- Single-agent (RAG) average usefulness score: `{create_single['avg_usefulness_score']}/5`",
...
def main() -> None:
    ...
    checkpoint_path = RESULTS_DIR / f"benchmark_single_rag_checkpoint_{args.prompt_count}.json"
    ...
    json_path = RESULTS_DIR / f"benchmark_single_rag_{timestamp}.json"
    md_path = RESULTS_DIR / f"benchmark_single_rag_{timestamp}.md"
